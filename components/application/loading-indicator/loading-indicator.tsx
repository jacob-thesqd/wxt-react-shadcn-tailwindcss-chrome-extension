...
const styles = {
  ...
  lg: {
    root: "gap-4",
    label: "text-lg font-medium text-muted-foreground",
    spinner: "h-14 w-14",
  },
  xl: {
    root: "gap-5",
...
export const LoadingIndicator = ({ type = "line-simple", size = "sm", label }: LoadingIndicatorProps) => {
  const renderSpinner = () => {
    ...
    if (type === "dot-circle") {
      return (
        <svg className={cn("animate-spin text-primary", styles[size].spinner)} viewBox="0 0 36 36" fill="none">
          <path
    ...
    return (
      <svg className={cn("animate-spin", styles[size].spinner)} viewBox="0 0 32 32" fill="none">
        <circle className="text-muted-foreground/30" cx="16" cy="16" r="14" stroke="currentColor" strokeWidth="4" />
        <circle
          className="stroke-primary"
          cx="16"
          cy="16"
  ...
  return (
    <div className={cn("flex flex-col items-center justify-center", styles[size].root)}>
      {renderSpinner()}
      {label && <span className={styles[size].label}>{label}</span>}
    </div>
  );
};
